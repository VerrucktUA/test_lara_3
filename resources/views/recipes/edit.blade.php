        ...
        <form action="{{ isset($recipe) ? route('recipes.update', $recipe->id) : route('recipes.store') }}"
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="form-control"
                          required>{{ isset($recipe) ? $recipe->description : '' }}</textarea>
            </div>

            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="status" name="status"
                       value="1" {{ isset($recipe) && $recipe->status ? 'checked' : '' }}>
                    ...
                    <tr>
                        <td>
                            <select name="existing_ingredient[]" required>
                                ...
                                @foreach ($ingredients as $ingredientl)
                                    @if ($ingredientl->id == $ingredient->id )
                                        <option value="{{ $ingredientl->id }}" selected>{{ $ingredientl->name }}</option>
                                    @else
                                        <option value="{{ $ingredientl->id }}">{{ $ingredientl->name }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </td>
            ...
            <button type="submit"
                    class="btn btn-primary">{{ isset($recipe) ? 'Update Recipe' : 'Create Recipe' }}</button>
        </form>
    </div>
@endsection
